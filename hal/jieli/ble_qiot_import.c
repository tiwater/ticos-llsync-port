...
int ble_get_product_id(char *product_id)
{
    ...
    memcpy(product_id, d_product_id, strlen(d_product_id));
...
int ble_get_device_name(char *device_name)
{
    ...
    memcpy(device_name, d_device_name, strlen(d_device_name));
...
int ble_get_psk(char *psk)
{
    ...
    memcpy(psk, d_secrt_key, strlen(d_secrt_key));
    put_buf(psk, strlen(psk));
    return 0;
}

int ble_set_dev_info(char *product_id, char *device_name, char *psk)
{
    memcpy (d_product_id, product_id, strlen(product_id));
    memcpy (d_device_name, device_name, strlen(device_name));
    memcpy (d_secrt_key, psk, strlen(psk));
    return 0;
}
